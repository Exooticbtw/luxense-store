...
export default function GuaranteeSection({ shopData }) {
  ...
  return (
    <section id="guarantee" style={{ padding: "0 24px 86px", background: "var(--bg)", scrollMarginTop: 110 }}>
      <div style={{ maxWidth: 1560, margin: "0 auto", borderRadius: 28, padding: 24, border: "1px solid rgba(17,17,17,.08)", background: "rgba(255,255,255,.86)", boxShadow: "0 18px 44px rgba(17,17,17,.05)" }}>
        <div style={{ display: "grid", gridTemplateColumns: ".95fr 1.05fr", gap: 24, alignItems: "center" }}>
          <div>
            <p className="eyebrow" style={{ color: "var(--accent)" }}>
              {guarantee.guaranteeEyebrow}
            </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 14 }}>
            {[
              ...
              <div key={label} style={{ minHeight: 118, borderRadius: 22, border: "1px solid rgba(17,17,17,.08)", background: "rgba(17,17,17,.96)", color: "var(--cream)", display: "grid", placeItems: "center", gap: 10, padding: 18, textAlign: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 14, marginTop: 18 }}>
          {guarantee.items.map((point) => (
            <div key={point.title} style={{ borderRadius: 20, padding: 18, background: "rgba(17,17,17,.04)", color: "var(--fg)", fontSize: 14, fontWeight: 700, lineHeight: 1.5 }}>
              <div style={{ fontWeight: 800 }}>{point.title}</div>
              <div style={{ marginTop: 6, color: "var(--muted)", fontWeight: 600 }}>{point.text}</div>
            </div>
          ))}
        </div>
      </div>
